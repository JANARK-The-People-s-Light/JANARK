...
const links = [
  ...
  { href: "/dashboard", label: templates.portal().pulseNav },
...
export function SiteHeader() {
  ...
  const mobileMenu =
    ...
    createPortal(
      ...
      >
        ...
        />
        <div className="absolute inset-y-0 right-0 flex w-[min(100%,20rem)] flex-col bg-chrome pt-[env(safe-area-inset-top,0px)] pb-[env(safe-area-inset-bottom,0px)] shadow-2xl">
          <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
            <BrandLogo size="sm" withWordmark onDark priority={false} />
            <button
              type="button"
              className="inline-flex h-11 w-11 items-center justify-center text-on-chrome"
              aria-label="Close menu"
              onClick={() => setMenuOpen(false)}
            >
              ✕
                  ...
                  <Link
                    ...
                    className={`block px-4 py-3.5 text-base ${
                      isActive(link.href)
                        ...
                        : "text-on-chrome active:bg-white/5"
                    }`}
                  >
                    {link.label}
                  </Link>
              ...
              {session ? (
                <div className="space-y-3">
                  ...
                  <button
                    type="button"
                    onClick={logout}
                    className="w-full border border-cream/30 px-4 py-3 text-sm text-on-chrome"
                  >
                    Log out
                  </button>
                </div>
              ) : (
                <p className="text-xs leading-relaxed text-on-chrome/60">
  ...
  return (
    <>
      <header className="fixed inset-x-0 top-0 z-50 border-b border-white/10 bg-chrome/95 backdrop-blur-md pt-[env(safe-area-inset-top,0px)]">
        <div className="mx-auto flex h-14 max-w-6xl items-center justify-between gap-3 px-4 sm:px-6">
          ...
          >
            ...
            {session ? (
              <span className="flex items-center gap-2 text-xs text-on-chrome/70">
                {session.anonId ? (
                  <Link
                    href={portalHref(`/u/${session.anonId}`)}
          ...
          <div className="flex items-center gap-2 lg:hidden">
            ...
            <button
              type="button"
              className="inline-flex h-11 w-11 items-center justify-center border border-cream/25 text-on-chrome"
        ...
        <div
          className="border-t border-amber/25 bg-amber text-center text-on-amber"
          role="status"
        >
          <p className="px-4 py-2 text-xs font-semibold tracking-wide sm:text-sm">
            Coming soon ·{" "}
            <time dateTime={PUBLIC_LAUNCH_ISO}>{PUBLIC_LAUNCH_LABEL}</time>
...
export function SiteFooter() {
  ...
  return (
    <footer className="mt-auto border-t border-line bg-chrome text-on-chrome pb-[env(safe-area-inset-bottom,0px)]">
      <div className="mx-auto flex max-w-6xl flex-col gap-4 px-4 py-8 sm:flex-row sm:items-start sm:justify-between sm:gap-8 sm:px-6">
        <div className="min-w-0">
          ...
          <p className="mt-2 max-w-md text-sm leading-relaxed text-on-chrome/70">
            {BRAND_TAGLINE}. {BRAND_SUPPORT}
          </p>
          <p className="mt-3 text-xs text-on-chrome/40">
        ...
        <div className="shrink-0 space-y-2 text-xs leading-relaxed text-on-chrome/50 sm:max-w-[14rem] sm:text-right">
          <p>
