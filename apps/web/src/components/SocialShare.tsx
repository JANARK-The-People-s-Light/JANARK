...
const PLATFORMS: {
  ...
  { id: "chat", label: "Chat", icon: <IconShareChat /> },
  { id: "feed", label: "Feed", icon: <IconShareFeed /> },
  { id: "network", label: "Network", icon: <IconShareNetwork /> },
  { id: "post", label: "Post", icon: <IconSharePost /> },
  { id: "stories", label: "Stories", icon: <IconShareStories /> },
...
export function SocialShare({
  ...
  async function onShare(platform: SocialPlatform) {
    ...
    if (platform === "stories") {
      await copyLink();
      window.open(urls.stories, "_blank", "noopener,noreferrer");
      setStatus("Link copied — ready to paste where you share");
      onDone?.();
      return;
    }

    const target = urls[platform as keyof typeof urls];
