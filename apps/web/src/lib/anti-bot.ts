...
export async function guardAnonymousWrite(
  ...
  if (opts.requireTurnstile && turnstileConfigured()) {
    // Official native clients cannot embed Turnstile WebView in OTP yet —
    // trust Origin-checked Janark-Android / Janark-iOS User-Agents instead.
    const ua = req.headers.get("user-agent") ?? "";
    const nativeClient =
      ua.startsWith("Janark-Android/") || ua.startsWith("Janark-iOS/");
    if (!nativeClient) {
      const token =
