...
/** Bump only citizen topic hashtags (trending topics). */
...
/**
 * Best-effort Mongo feed card after a canonical SQLite write.
 * Never throws — SQLite remains source of truth; home feed may lag until retry/ops.
 */
export async function mirrorFeedCard(
  doc: Parameters<typeof FeedPost.create>[0],
): Promise<void> {
  try {
    await FeedPost.create(doc);
  } catch (err) {
    console.error("[janark] dual-write FeedPost failed", err);
  }
}
