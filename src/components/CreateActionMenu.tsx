...
export function CreateActionMenu() {
  ...
  return (
    <div
      ref={rootRef}
      className="fixed bottom-[calc(4.25rem+env(safe-area-inset-bottom,0px))] right-[max(1rem,env(safe-area-inset-right))] z-[90] flex flex-col items-end lg:bottom-[max(1.25rem,env(safe-area-inset-bottom))]"
    >
      <input
        ref={fileRef}
        type="file"
      ...
      {uploadError ? (
        <p className="mb-2 max-w-[15rem] border border-danger/30 bg-white px-3 py-2 text-xs text-danger shadow-lg">
          {uploadError}
        </p>
      ) : null}
      ...
      {open ? (
        <div
          id={menuId}
          role="menu"
          className="mb-2 min-w-[15rem] border border-line bg-white py-2 text-navy shadow-xl"
        >
          <p className="px-4 pb-1 pt-1 text-[11px] font-medium uppercase tracking-wide text-amber">
            Quick
          </p>
          <ul>
            <li role="none">
              <button
                ...
                role="menuitem"
                onClick={startShare}
                className="block w-full px-4 py-2.5 text-left text-sm text-navy hover:bg-cream hover:text-amber"
              >
                Share
              </button>
            </li>
          ...
          <p className="px-4 pb-1 pt-1 text-[11px] font-medium uppercase tracking-wide text-fact">
            Organize
          </p>
          <ul>
            {ORGANIZE.map((a) => (
      ...
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        disabled={uploading}
        className="flex h-12 w-12 items-center justify-center bg-amber text-2xl font-semibold leading-none text-navy shadow-lg transition hover:bg-amber-bright disabled:opacity-70"
      >
