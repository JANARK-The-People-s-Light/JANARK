...
export default function NewDemandPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    const voterKey = ensureAuth("raise this public demand");
    if (!voterKey) return;
    setSaving(true);
    setError(null);
    try {
      ...
      router.push(portalHref(`/demands/${data.demand.id}`));
    } catch {
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-12 sm:px-6">
      ...
      <h1 className="font-display mt-2 text-3xl text-navy">
        Raise a public demand
      </h1>
      <p className="mt-3 text-sm text-muted">
        Draft freely. Login with phone OTP appears when you publish — we keep
        complete anonymity.
      </p>
      ...
      >
        ...
        >
          {saving ? "Publishing…" : "Publish demand"}
        </button>
      </form>
    </div>
  );
